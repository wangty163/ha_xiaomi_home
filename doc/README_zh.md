...
当“房间名同步模式”选择“家庭名”“房间名”或“家庭名和房间名”时，集成会让 Home Assistant 中已有设备的区域持续与米家当前家庭及房间信息保持一致。集成会在启动时以及每小时刷新一次米家设备列表；如果对应区域不存在则自动创建，并在设备的米家房间发生变化后移动设备。选择“不同步”可关闭此行为。实体单独设置的区域不会被修改。
